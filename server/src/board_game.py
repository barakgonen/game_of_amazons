    ...
    def __init__(self, size, white_amazons=None, black_amazons=None, blockers_pos=None):
        ...
        if white_amazons is None and black_amazons is None and blockers_pos is None:
            ...
            for white_amazon in white_amazons:
                self.board[get_raw_index(white_amazon.y, self.size)][get_col_index(white_amazon.x, self.size)].state = CellState.WHITE_AMAZON
            for black_amazon in black_amazons:
                self.board[get_raw_index(black_amazon.y, self.size)][get_col_index(black_amazon.x, self.size)].state = CellState.BLACK_AMAZON
            for blocker_pos in blockers_pos:
                self.board[get_raw_index(blocker_pos.y, self.size)][get_col_index(blocker_pos.x, self.size)].state = CellState.BLOCKED
