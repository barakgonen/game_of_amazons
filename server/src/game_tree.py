...
class GameTree:
    ...
    def __init__(self,
        ...
        self.root = GameNode(self.board_size, white_amazons_position, black_amazons_position,
                             self.blocking_rocks_position, self.available_steps_manager)
    ...
    def generate_possible_board_states_for_amazona(self, amazona, searching_depth, player_color, current_white_amazons,
                                                   current_black_amazons, current_blocking_rocks_pos):
        available_states_for_amazona = []
        available_steps_for_amazona = self.available_steps_manager.get_available_moves_for_amazona(self.board_size,
        ...
        for available_move in available_steps_for_amazona:
            ...
            if player_color == "BLACK":
                ...
                available_throwing_rocks_options = self.blocking_rocks_manager.generate_available_throwing_options(
                    self.board_size, current_white_amazons, players_current_pos, current_blocking_rocks_pos,
                    available_move, self.searching_distance)
            elif player_color == "WHITE":
                players_current_pos = copy.deepcopy(current_white_amazons)
                players_current_pos.remove(amazona)
                players_current_pos.append(available_move)
                available_throwing_rocks_options = self.blocking_rocks_manager.generate_available_throwing_options(
                    self.board_size, players_current_pos, current_black_amazons, current_blocking_rocks_pos,
                    available_move, self.searching_distance)
            else:
            ...
            for available_throwing_option in available_throwing_rocks_options:
                ...
                if player_color == "BLACK":
                    turn_game_node = GameNode(self.board_size, current_white_amazons, players_current_pos,
                                              throwing_option, self.available_steps_manager)
                elif player_color == "WHITE":
                    turn_game_node = GameNode(self.board_size, players_current_pos, current_black_amazons,
                                              throwing_option, self.available_steps_manager)
    ...
    def generate_my_amazons_next_possible_move_and_shot(self, color):
        ...
        if color == "WHITE":
            for amazona in self.white_amazons_position:
                available_stetes_for_amazona = self.generate_possible_board_states_for_amazona(amazona,
        ...
        print("TOOK JUST: ", str(total_time.seconds) + "." + str(
            total_time.microseconds) + " seconds" + " number of nodes in depth 1 is: " + str(len(self.root.children)))
