...
def thread_function(status, white, black, blocking_rocks, flag, name="AA"):
    logging.info("Thread %s: starting", name)
    logging.info("Thread %s: name: ", name)
    logging.info("Thread %s: status", status)
    for w in white:
        logging.info("Thread %s: White " + str(w))
    for b in black:
        logging.info("Thread %s: Black" + str(b))
    for r in blocking_rocks:
        logging.info("Thread %s: blocking" + str(r))
    logging.info("Thread %s: flag", flag)
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for i in range(0, 10):
            white_random = random.randrange(1, 10)
            black_random = random.randrange(1, 20)
            blocking_random = random.randrange(1, 50)
            white = []
            black = []
            blk = []
            thread_index = "ID-" + str(index)
            index += 1
            for i in range(0, white_random):
                white.append(i)
            for i in range(0, black_random):
                black.append(i)
            for i in range(0, blocking_random):
                blk.append(i)
            flag = not flag
            future_to_url ={executor.submit(thread_function, url, white, black, blk, flag, thread_index) : url for url in URLS}
